...
var p = function(core, config, state) {
  ...
  self.cb = function (arg, payload) {
    if(moment().diff(moment(state['lastcheck'])) < config.cooldown){
      self.core.say(state['lastresult'])
      return
    }
    request.get({ url: config.endpoint, json: true }, function (error, response, data) {
      if (error || response.statusCode !== 200) {
        console.log("HTTP error in SC2 Plugin SoSad ", error)
        self.core.say("HTTP error in SC2 Plugin SoSad " + error)
        return
      }
